...
from model import Post, MentionedAuthor, MentionedBrand, MentionedProduct
...
with Session(engine) as session:
    ...
    statement = (
        select(MentionedAuthor.author, func.count(MentionedAuthor.author).label('mention_count'))
    ...
    statement = (
        select(MentionedBrand.brand, func.count(MentionedBrand.brand).label('mention_count'))
    ...
    statement = (
        select(MentionedProduct.product, func.count(MentionedProduct.product).label('mention_count'))
        .group_by(MentionedProduct.product)
        .order_by(desc('mention_count'))
        # .limit(10)
    )
    top_products = session.exec(statement).all()
    export_to_csv("top_mentioned_products.csv", top_products, ["Product", "Mentions"])
    ...
    export_to_csv("posts_per_author.csv", posts_per_author, ["Author", "Post Count"])
